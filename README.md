...
Прототип веб‑приложения для ускорения учета авиационных инструментов на этапах выдачи и возврата. Решение включает минималистичный FastAPI бэкенд, который хранит сессии сверки, принимает изображения и обращается к модулю детекции (пока имитирован), а также одностраничный фронтенд без внешних зависимостей.
...
- Готовый коннектор для подключения внешнего CV‑сервиса (например, FastAPI + YOLOv10) через REST.
...
backend/
  ...
  app/
    ...
    services/
      detection_client.py # Mock и HTTP-клиент детектора
...
frontend/
  ...
  styles.css
  app.js              # Логика: создание сессии, загрузка изображений, визуализация
...
Модуль `backend/app/services/detection_client.py` содержит два клиента:
1. `MockDetectionClient` — текущее поведение (детерминированная заглушка).
2. `HttpDetectionClient` — готовый клиент для внешнего FastAPI сервиса. Ожидает конечную точку `POST /detect`, принимающую файл `file` и возвращающую JSON вида:
   ```json
   {
     "detections": [
   ...
   ```
Чтобы переключиться на реальный детектор, задайте `DETECTION_SERVICE_URL=http://host:port` перед запуском сервера.

## Пользовательский сценарий
1. Настроить сессию (режим, порог, список инструментов).
2. Получить ID сессии и подсказки о статусе.
3. Загрузить фотографию рабочего стола — результат появится автоматически.
4. Проверить процент совпадения, списки отсутствующих и посторонних инструментов.
5. При необходимости зайти в `/admin`, авторизоваться и свериться с историей предыдущих сессий.
...
## Лицензия
Проект можно использовать как отправную точку для дальнейшей проработки пилота. Убедитесь, что внешние зависимости соответствуют требованиям локального законодательства и корпоративных политик.
